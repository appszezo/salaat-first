...
public class ArabicReshaper {
	...
	class DecomposedWord {
		...
		DecomposedWord(String unshapedWord) {
			...
			for (int index = 0; index < unshapedWord.length(); index++) {
				if (isHaraka(unshapedWord.charAt(index))) {
					...
					stripedHarakates[harakatesCount] = unshapedWord
							.charAt(index);
					harakatesCount++;
				} else {
					lettersPositions[letterCount] = index;
					stripedRegularLetters[letterCount] = unshapedWord
							.charAt(index);
					letterCount++;
				}
			}
		...
		String reconstructWord(String reshapedWord) {
			...
			wordWithHarakates = new char[reshapedWord.length()
					+ stripedHarakates.length];
			for (int index = 0; index < lettersPositions.length; index++) {
				wordWithHarakates[lettersPositions[index]] = reshapedWord
						.charAt(index);
			}
	...
	public static char[][] LAM_ALEF_GLPHIES = { { 15270, 65270, 65269 },
			{ 15271, 65272, 65271 }, { 1575, 65276, 65275 },
			{ 1573, 65274, 65273 } };
	...
	public ArabicReshaper(String unshapedWord) {
		...
		if (decomposedWord.stripedRegularLetters.length > 0) {
			_returnString = reshapeIt(new String(
					decomposedWord.stripedRegularLetters));
		}
	 ...
	 */
	private char getLamAlef(char candidateAlef, char candidateLam,
			boolean isEndOfWord) {
	...
	private String replaceLamAlef(String unshapedWord) {
		...
		for (int index = 0; index < wordLetters.length - 1; index++) {
			if (!isHaraka(wordLetters[index])
					&& DEFINED_CHARACTERS_ORGINAL_LAM != wordLetters[index]) {
			...
			if (DEFINED_CHARACTERS_ORGINAL_LAM == wordLetters[index]) {
				...
				while (harakaPosition < wordLetters.length
						&& isHaraka(wordLetters[harakaPosition])) {
					harakaPosition++;
				}
				if (harakaPosition < wordLetters.length) {
					char lamAlef = 0;
					if (lamPosition > 0 && getGlphyType(letterBefore) > 2)
						lamAlef = getLamAlef(wordLetters[harakaPosition],
								candidateLam, false);
					else {
						lamAlef = getLamAlef(wordLetters[harakaPosition],
								candidateLam, true);
	...
	public String reshapeIt(String unshapedWord) {
		...
		if (wordLength >= 2) {
			if (getGlphyType(wordLetters[wordLength - 2]) == 2) {
				// If the letter has only 2 shapes, then it doesnt matter which
				// position it is, It'll be always the second form
				reshapedWord.append(getReshapedGlphy(
						wordLetters[wordLength - 1], 1));
			} else {
				// Put the right form of the character, 4 for the last letter in
				// the word
				reshapedWord.append(getReshapedGlphy(
						wordLetters[wordLength - 1], 4));
			}
		}
