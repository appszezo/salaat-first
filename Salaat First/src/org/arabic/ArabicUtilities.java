...
public class ArabicUtilities {
	...
	private static boolean isArabicCharacter(char target) {
		String s=String.format("\\u%04x", (int) target);
	...
	public static String reshape(int resID) {
		String res = SalaatFirstApplication.getLastInstance().getString(resID);
		return reshape(res);
	}
	...
	public static String reshapeSentence(int resID) {
		Resources res=SalaatFirstApplication.getLastInstance().getResources();
		res.updateConfiguration(lastConfiguration, res.getDisplayMetrics());
		String sentence = res.getString(
				resID);
		return reshapeSentence(sentence);
	}
	...
	public static String[] reshapeText(String[] str) {
		...
		for (int i = 0; i < str.length; i++) {
			output[i] = reshapeSentence(str[i]);
		}
		return output;
	}
}
