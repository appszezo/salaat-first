...
/**
 * This the main class of the JIslamic library. You can use static methods to do
 * qibla calculation but you will need to create a JIslamic instance to
 * calculate prayer times for a specific location, using a specified method.
 * 
 */
public class JIslamic {
		...
		A = astro.getRa()[1]
				+ (M
						* ((astro.getRa()[1] - ra0) + (ra2 - astro.getRa()[1]) + ((ra2 - astro
								.getRa()[1]) - (astro.getRa()[1] - ra0)) * M) / 2.0);
	...
	/*
	 * This is Used for storing some formulae results between multiple
	 * getPrayerTimes calls
	 */
	 ...
	 *            the location
	 * @param method
	 *            the method used in the calculation. You can use predefined
	 *            methods for example <code>Method.MUSLIM_LEAGUE</code> or
	 *            creates your own personalized method.
	 */
	...
	void base6hm(double bs, Method method, Prayer pt, PrayerTime type) {
		...
		} else if (method.getRound() == Rounding.SPECIAL
				|| method.getRound() == Rounding.AGRESSIVE) {
	...
	public Prayer getImsaak(SimpleDate date) {
		 ...
		 */
		/*
		 * In case of an extreme Fajr time calculation use intervals for Imsaak
		 * and compute again
		 */
	 ...
	 * @param date
	 *            the date of prayers
	 * @return a DayPrayers object containing time of different prayers
	 */
	 ...
	 *            the date of prayers
	 * @param pt
	 *            instance of a DayPrayers object
	 */
	...
	/*
	 * Obtaining the direction of the shortest distance towards Qibla by
	 * uMath.sing the great circle formula
	 */
	...
	void getPrayerTimesByDay(Method method, DayCouple dc, DayPrayers pt,
		...
		/*
		 * Start by filling the tAstro structure with the appropriate
		 * astronomical values for this day. We also pass the cache structure to
		 * update and check if the actual values are already available.
		 */
		...
		/*
		 * Calculate all prayer times as Base-10 numbers in Extreme Latitudes
		 * (if needed)
		 */
		...
		if ((method.getExtremeLatitude() != ExtremeLatitude.NONE_EX)
			...
			if (ext == ExtremeLatitude.LAT_ALL
					|| ext == ExtremeLatitude.LAT_ALWAYS
					|| ext == ExtremeLatitude.LAT_INVALID) {
				/*
				 * case LAT_ALL: case LAT_ALWAYS: case LAT_INVALID:
				 */

				/*
				 * xxxthamer: we cannot compute this when interval is set
				 * because angle==0 . Only the if-invalid methods would work
				 */
				exLoc.setDegreeLat(method.getNearestLat());
				exFj = getFajIsh(method.getNearestLat(), dec,
						method.getFajrAng());
				// exIm = getFajIsh(method.getNearestLat(), dec,
				// method.getImsaakAng());
				exIs = getFajIsh(method.getNearestLat(), dec,
						method.getIshaaAng());
			...
			} else if (ext == ExtremeLatitude.GOOD_ALL
					|| ext == ExtremeLatitude.GOOD_INVALID
				...
				if (ext == ExtremeLatitude.GOOD_ALL) {
					...
					/*
					 * Nearest Good Day: Different good days for Fajr and Ishaa
					 * (Not implemented)
					 */
				}
			} else if (ext == ExtremeLatitude.SEVEN_NIGHT_ALWAYS
					|| ext == ExtremeLatitude.SEVEN_NIGHT_INVALID
					|| ext == ExtremeLatitude.SEVEN_DAY_ALWAYS
					|| ext == ExtremeLatitude.SEVEN_DAY_INVALID
					|| ext == ExtremeLatitude.HALF_ALWAYS
					|| ext == ExtremeLatitude.HALF_INVALID) {

				/*
				 * xxxthamer: For clarity, we may need to move the HALF_*
				 * methods into their own separate case statement.
				 */

				if (ext == ExtremeLatitude.SEVEN_NIGHT_ALWAYS
						|| ext == ExtremeLatitude.SEVEN_NIGHT_INVALID) {
					portion = (24 - (tempPrayer[4] - tempPrayer[1]))
							* (1 / 7.0);
				} else if (ext == ExtremeLatitude.SEVEN_DAY_ALWAYS
						|| ext == ExtremeLatitude.SEVEN_DAY_INVALID) {
					portion = (tempPrayer[4] - tempPrayer[1]) * (1 / 7.0);
				} else if (ext == ExtremeLatitude.HALF_ALWAYS
						|| ext == ExtremeLatitude.HALF_INVALID) {
				...
				/*
				 * Do nothing here because this is implemented through fajrInv
				 * and ishaaInv structure members
				 */
				...
				if (tempPrayer[0] == 99) {
					exinterval = (int) (method.getFajrInv() / 60.0);
					tempPrayer[0] = tempPrayer[1] - exinterval;
					pt.fajr().setExtreme(true);
				}
				if (tempPrayer[5] == 99) {
					exinterval = (int) (method.getIshaaInv() / 60.0);
		...
		/*
		 * Final Step: Fill the Prayer array by doing decimal degree to Prayer
