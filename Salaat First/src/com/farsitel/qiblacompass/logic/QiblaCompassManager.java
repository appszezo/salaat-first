...
class LocationChangedAsyncTask extends AsyncTask<Location, Void, Double> {
	...
	protected void onPostExecute(Double result) {
		...
		message.what = ConstantUtilInterface.MESSAGE_ON_QIBLA_DIRECTION;
		Bundle bundle = new Bundle();
		bundle.putDouble(ConstantUtilInterface.MESSAGE_DEVICE_LOCATION_KEY,
				result.doubleValue());
...
public class QiblaCompassManager implements SensorEventListener,
	...
	private boolean isLocationFromGPS = true;
	private final QiblaFragment qiblaFragment;
		...
		public void handleMessage(Message message) {
			...
			case MESSAGE_ON_DEVICE_HEAD_DIRECTION:
				...
				try {
					...
					isNorthChanged = true;
					qiblaFragment.signalForAngleChange();
	...
	public QiblaCompassManager(QiblaFragment qiblaFragment) {
	...
	public void onLocationChanged(Location location) {
		...
		if (previousLocation == null || isFarEnough(location, previousLocation)) {
			qiblaFragment.onNewLocation(location, isLocationFromGPS);
	...
	public void setLocationFromGPS(boolean isLocationFromGPS) {
		this.isLocationFromGPS = isLocationFromGPS;
	}
}
