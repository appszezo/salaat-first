...
/**
 * Reference:   Formulas from the "Astronomical Algorithms", Jean Meeus; First Edition, 1991; Willmann -Bell Inc; Richmond, USA.
 * each  formula is referred to the chapter of this book
 * 
 * Thanks to Mr Moura Mkhakh for his help.
 */
...
public class PrayerTimesCalculator {
	...
	private double mElev=0;  // level from sea, using this default value in this version
	private double offsets[]={0,0,0,0,0,0};
	//---astronomic fields
	private double Rv_s,       	 //  Sun's radius vector= distance from Earth to the Sun given in astronomical units
    sunSemiD,  		      //  Sun semidiameter
    decl;       		     //  Sun' Declination
	
	/*private double alfa,                       //  Apparent Right Ascension of the Sun in Degree
    ...
    Ma_s,          				  //  Mean Anomaly in degree of the sun
    Ma_m,        		     //  Moon mean anomaly
    i_m,         		     //  Phase angle of moon
    D_m,                        //  Mean ellogation of the moon
    Ect,        		     //  Eccentricity of Earth's orbit
    c,          		     //  Suns's equation of center
    v,          		     //  Suns True Anomaly
    omega,      		      //  Longitude of the ascending node of the moon's mean orbit on the ecliptique
    DeltaPhi,   		      //  Nutation  in longitude
    
    EqT,        		     //  Eqaution Of Time  expressed in minutes of time*/
   
	
	public PrayerTimesCalculator(double longitude, double latitude, Organization org,
			double timeZone, int asrMadhab,double level, double[] offsets)
	{
	...
	/**
	 * @param mAsrMadhab the mAsrMadhab to set
	 */
	...
	/**
	 * @param mOrganization the Organization to set
	 */
    ...
    {
    	double cosH0= ((cos(Z*PI/180)- sin(mLatitude* PI/180)*sin(decl* PI/180))
	...
	public void performCalculs(Calendar cal) {
		...
		double T4 = T3 * T;
		double T5 = T4 * T;
		double L0 = (280 + 27.0 / 60 + 59.244 / 3600) + (1296027713.8 / 3600) * T // sun's
																			// mean
																			// Longitude
																			// in
																			// degree
		...
		double L_m = 218.3164591 + 4812678.8134236 * T - 0.13268 * T2 + T3 / 538.841
				- T4 / 6519.4;
		L_m = fixangle(L_m);
		double Ma_s = 357.5291092 + 359990.502909 * T - 0.01559 * T2 - T3 / 24490;
		Ma_s = fixangle(Ma_s);

		// Obliquity Of Ecliptique in dergree (21.2)
		double epsilon = (23 + 26.0 / 60 + 21.448 / 3600) - 468.15 / 3600 * T - 0.059
				/ 3600 * T2 + 0.1813 / 3600 * T3;
		...
		double DeltaPhi = -17.20 / 3600 * sin(omega * PI / 180) - 1.32/ 3600 * sin(2 * L0 * PI / 180) - 0.23 / 3600
		...
		decl = asin(sin(epsilon * PI / 180)
				* sin(lambda * PI / 180))
				* 180 / PI;

		// Sun's radius vector= distance from Earth to the Sun given in
		// astronomical units
		Rv_s = 1.000001018 * (1 - sqrt(Ect)
				/ (1 + Ect * cos(v * PI / 180))); // (24.5)
		...
		// (27.1)
		double EqT = L0 - 0.0057183 - alfa + DeltaPhi
				* cos(epsilon * PI / 180);
	...
	{
		double fajr=mDhuhr - hourAngle(mOrganization.getZenithDawn())/15+offsets[FAJR];
		return fajr;
	}
	...
	public double getChoroukTime()
	{
	    ...
	    R= R* P/1010 * 283.0/(273+T);          // correction of temperature and pression
	    R= R/60;                             // translate from minutes to degree
	    double zenithSunRise= 90 + sunSemiD + R -8.794/(3600 * Rv_s) +0.035333* sqrt(mElev);
	...
	{
		double a= asin(sin(mLatitude* PI/180)*sin(decl* PI/180) + cos(mLatitude* PI/180)*cos(decl* PI/180))* 180/PI;
		double Zenith_AfterNoon;
	if(mAsrMadhab== SHAFII)
	    Zenith_AfterNoon= 90 - acot(1 + (1/tan(a* PI/180)))* 180/PI;      //  ــــــــ    :الفقه شافعي
	    else
	    Zenith_AfterNoon= 90 - acot(2 + (1/tan(a* PI/180)))* 180/PI;     //  ــــــــ   :الفقه حنفي
	...
	public double getMaghribTime()
	{
	  ...
	  double Zenith_SunSet= 90 + sunSemiD + R -8.794/(3600 * Rv_s) +0.035333* sqrt(mElev);
	  // (14.2)
	...
	public double getIchaaTime()
	{
		if(mOrganization == Organization.UQ)
		    ...
		    {
			double ichaa=mDhuhr + hourAngle(mOrganization.getZenithDusk())/15+offsets[ICHAA];
			return ichaa;
		    }
	}
...
public static enum Organization
{
	...
	EGOS(109.5,107.5),       //  - Egyptian General Organization of surveying 19.5º /17.5º
    //  الهيئة العامة المصرية للمساحة   .
	UQ(109,90);          //  - Umm al-Qura kalender 19º for fajr, and maghrib+90min for icha (+120min in ramadan)
    //  جامعة أم القرى .
 ...
 * @param prayer the order of the prayer
 * @param context the context of application
 * @return
 */
public static String getPrayerName(int prayer, Context context)
{
	MainActivity.validateLanguage(context);
	switch(prayer)
	{
	case FAJR:
		return reshapeText(context.getString(R.string.fajr_name),isReshapingNessecary);	
	case CHOROUK:
		return reshapeText(context.getString(R.string.sunrise_name),isReshapingNessecary);
	case DHUHR:
		return reshapeText(context.getString(R.string.dhuhr_name),isReshapingNessecary);
	case ASR:
		return reshapeText(context.getString(R.string.asr_name),isReshapingNessecary);
	case MAGHRIB:
		return reshapeText(context.getString(R.string.maghrib_name),isReshapingNessecary);
	case ICHAA:
		return reshapeText(context.getString(R.string.ishaa_name),isReshapingNessecary);
	}
	return null;
}
