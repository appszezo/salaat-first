...
public class FileChooserActivity extends Activity implements FileChooser {
	...
	private boolean asRingtoneManager;
	...
	public static final String INPUT_AS_RINGTONE_MANAGER = "as_ringtonemanager";
	// ---- Activity methods ----- //
    ...
    public void onCreate(Bundle savedInstanceState) {
        ...
        if(extras != null) {
            ...
            if(extras.containsKey(INPUT_AS_RINGTONE_MANAGER)) 
            	{
            	this.asRingtoneManager=true;
            	if(extras.containsKey(RingtoneManager.EXTRA_RINGTONE_TYPE))
            		this.ringtoneType=extras.getInt(RingtoneManager.EXTRA_RINGTONE_TYPE);
            	}
        }
        ...
        core.addListener(new FileChooserCore.OnFileSelectedListener() {
			public void onFileSelected(File folder, String name) {
				...
				intent.putExtra(RingtoneManager.EXTRA_RINGTONE_PICKED_URI, Uri.parse(folder+File.separator+name));
			...
			public void onFileSelected(File file) {
				...
				if(asRingtoneManager)
				{
				    ...
				    Uri newUri = getRingtoneUri(file);

					intent.putExtra(RingtoneManager.EXTRA_RINGTONE_PICKED_URI, newUri);
					setResult(RESULT_OK, intent);
					finish();
				}
				else
    ...
    private Uri getRingtoneUri(File file)
    {
    	...
    	Cursor cursor=getContentResolver().query(uri, new String[] { "_id" }, MediaStore.MediaColumns.DATA+"=?", new String[] { file.getAbsolutePath() }, null);
    	Uri newUri;
    	...
    	else
    	{
    		...
    		values.put(MediaStore.MediaColumns.SIZE, file.length()/1024);
    		values.put(MediaStore.MediaColumns.MIME_TYPE, "audio/*");
        	values.put(MediaStore.Audio.Media.IS_MUSIC, true); //the file is choosen from sdcard, so it should be possible to play it.
